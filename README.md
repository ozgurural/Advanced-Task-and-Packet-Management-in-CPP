...
This project contains a C++ class called PeriodicTask that represents a periodic task that can be executed at a specified interval. The TaskManager class is also provided, which manages a list of PeriodicTask objects and executes them at their specified intervals. In addition, the TaskManager class also manages a queue of packets, and processes them at the specified time.
...
To run the unit tests, first ensure that Google Test is installed on your system. Then, run CMake in the build directory with the -DBUILD_TESTS=ON flag:

Copy code
cmake -DBUILD_TESTS=ON ..
This will generate the necessary build files for the unit tests. To build and run the tests, run:

Copy code
make
make test
This will build the unit_tests executable and run all of the unit tests.
...
CMake - This is used to build the project.
...
To run the project, execute the following command: ./forescout

To run the unit tests, execute the following command: ./unit_tests
